...
# Enhanced chunked extraction for large datasets (moved to global for worker access)
extract_data_chunked <- function(text_data, extract_types, chunk_size = 10000) {
  ...
  # Convert to character vector robustly
  ...
  emails <- unique(emails[nzchar(emails)])
  phones <- unique(phones[nzchar(phones)])
        ...
        data <- tryCatch(
          ...
          error = function(e) {
            ...
            tryCatch(
              ...
              error = function(e2) {
                stop(paste(
                  "Failed to read CSV file. Please check the separator and file format.",
        ...
        tryCatch(
          {
            ...
            if (sheet > length(sheet_names)) {
              stop(paste(
                "Sheet", sheet, "does not exist. Available sheets:",
